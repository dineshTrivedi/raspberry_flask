...
-----
Web server - uwsgi, nginx, apache
sao usados para servirem arquivos estaticos na internet (via HTTP), isso porque se vc batesse na url de um arquivo php vc iria fazer download ao inves de ser servido como um site.
Ate onde li o uwsegi seria capaz de servir os arquivo, mas seria vivavel apenas se fosse um site com pouquissimo trafego.
Existe a combinação uwsgi + nginx, já que o nginx ainda pode fazer proxy reverso, cache, compressao de arquivos, controle de carga e outras features.
